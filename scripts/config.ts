...
const main = async () => {
    const schema = openJSON(path.resolve(process.cwd(), 'config', 'config.schema.json'), z.record(z.unknown())).unwrap();
    ...
    await fs.promises.writeFile(path.resolve(process.cwd(), configPath), JSON.stringify(config, null, 4));
