...
export default async (...args: string[]) => {
	...
	// target should look like git@github.com:tsaxking/sveltekit-template.git
	// target can point to any git server, not just github
	// ensure it is a valid git repository URL
	// const regex = /^(git@github\.com:|https:\/\/github\.com\/)([\w-]+)\/([\w-]+)(\.git)?$/;
	const regex = /^(git@[\w.-]+:|https:\/\/[\w.-]+\/)([\w-]+)\/([\w-]+)(\.git)?$/;
