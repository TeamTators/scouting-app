...
export default async (...args: string[]) => {
    ...
    for (const row of tableRows) {
        ...
        while (hasMore) {
            ...
            if (table_error) {
                console.error(`Error fetching data from ${schema_name}.${table_name}: ${table_error.message}`);
                break;
            }
    ...
    const file_name = export_name ? `supabase-export-${export_name}-${Date.now()}.zip` : `supabase-export-${Date.now()}.zip`;
