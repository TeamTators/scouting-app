...
import {
    ...
    Zones,
    Match
} from 'tatorscout-utils/trace';
import { Requests } from '../utils/requests';
import { alert, confirm } from '../utils/prompts';
import { Assignment } from 'tatorscout-utils/scout-groups';
import {
    CompLevel,
    matchSort,
    TBAEvent,
...
> {
    ...
    public static async getEventData(key?: string): Promise<Result<EventData>> {
        return attemptAsync(async () => {
            ...
            const res = await Requests.post<EventData>('/event-data', {
                body:  {
                    key: key || ''
                },
                expectStream: false,
...
sse.on('change-state', (obj) => {
    ...
    const { id, data: state } = res.data;
    // TODO: When metadata is public, uncomment this
    // if (id !== Requests.metadata.get('tablet-id')) return;
...
//     if (id === Requests.metadata.get('tablet-id')) App.abort();
// });
