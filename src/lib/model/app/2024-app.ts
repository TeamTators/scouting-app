...
import { amps, autoZone, border, notePositions, srcs, stages, zones } from 'tatorscout/2024-areas';
import { type Action2024, type TraceParse2024, type Zones2024 } from 'tatorscout/trace';
import { App } from './app';
...
export const generate2024App = (matchData: {
	...
	app.addAppObject(
		...
		new I('Blue Source', 'Robot retrieves a note from the blue source', 'src', 0),
		blueSource,
		(i) => i.toString(),
		'blue'
	);
	...
	app.addAppObject(
		...
		new I('Red Source', 'Robot retrieves a note from the red source', 'src', 0),
		redSource,
		(i) => i.toString(),
		'red'
	);
