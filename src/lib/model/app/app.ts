...
import { downloadText, loadFileContents } from '../../utils/downloads';
...
import { TabletStateSchema } from '../admin';
import { z } from 'zod';
...
> {
	...
	public static $events: TBAEvent[] = (() => {
		try {
			return z.array(EventSchema).parse(JSON.parse(window.localStorage.getItem('events') || '[]'));
		} catch (error) {
			console.error(error);
			return [];
		}
	...
	public static actionAnimation(icon: HTMLElement, alliance: 'red' | 'blue' | null) {
		...
		attempt(() => {
			...
			for (let i = 0; i < app.ticks.length; i++) {
				...
				try {
					...
					const obj = app.appObjects[p[3] as number] as AppObject<unknown, Action>;
	...
	static saveToLocalStorage(...matches: Match[]) {
		...
		window.localStorage.setItem(SAVED_MATCH_VERSION + '-savedMatches', JSON.stringify(saved));
	...
	constructor(
		...
		this.buttonCircle = new ButtonCircle<a>(this as unknown as App, this.matchData);
	 ...
	 * @type {*}
	 */
	public readonly buttonCircle: ButtonCircle<a>;
	 ...
	 * @returns {*}
	 */
	addArea(zone: z, points: Point2D[], color: Color, condition: (shape: Polygon) => boolean) {
	...
	public async build(): Promise<undefined | (() => void)> {
		...
		this.ticks = new Array(160 * App.ticksPerSecond)
			.fill(null)
			.map((_, i) => new Tick<a>(i * App.tickDuration, i, this as App<any, any>));
		...
		const currentAlliance = getAlliance({
			matches: this.matchData.matches,
			teamNumber: this.matchData.team,
			matchNumber: this.matchData.match,
			compLevel: this.matchData.compLevel
		});
	...
	pull(): TraceArray {
		return this.ticks
			.map((t, i) => {
				let p = t.point;
				if (p) {
					p = [App.flipX ? 1 - p[0] : p[0], App.flipY ? 1 - p[1] : p[1]];
				} else {
					p = [-1, -1];
				}
				const [x, y] = p;
