...
export class MatchData {
	...
	public async getAlliance(): Promise<'red' | 'blue' | null> {
		...
		const match = matches.find(
			(m) => m.match_number === this.matchNumber && m.comp_level === this.compLevel
		);
	...
	async selectGroup(group: number, matchIndex?: number, doConfirm = true) {
		return attemptAsync(async () => {
			...
			const changeGroup = async (num: number) => {
				...
				this.teamNumber = eventData.value.assignments.matchAssignments[num][index];
	...
	async moveIndex(movingMatchIndex: number) {
		return attemptAsync(async () => {
			...
			if (group === -1) {
				...
				this.teamNumber = eventData.value.assignments.matchAssignments[group][currentMatchIndex];
			}
