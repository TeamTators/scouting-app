...
class Button {
	...
	constructor(public readonly app: App, public readonly config: ButtonConfig) {
		...
		if (browser) {
			...
			this.el.classList.add('btn', config.alliance === 'red' ? 'btn-danger' : 'btn-primary', 'circle', 'p-0');
			this.el.onclick = () => {};
...
export class ButtonCircle extends Drawable<ButtonCircle> {
	...
	addButton(config: ButtonConfig) {
		...
		const click = () => {
			...
			if (button.iterator.state !== undefined) this.app.state.tick?.set(new ActionState({
