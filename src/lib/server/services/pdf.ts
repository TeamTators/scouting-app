...
export class PDF {
    ...
    /**
     * Save PDF using an atomic write strategy (temp file then rename).
     *
     * Writes to a temporary file first, then atomically renames it to the target path.
     * Reduces risk of partial/corrupt files if the process crashes. Ideal for production
     * use where data integrity is critical.
     *
     * @param {string} filePath Absolute output path. Must start with `/` (absolute path required).
     * @param {PDFSaveOptions} [options={}] Save options (directory creation, file mode).
     * @returns {import('ts-utils').ResultPromise<void, Error>} Async save result.
     * @example
     * const atomicResult = await pdf.saveAtomic('/srv/reports/monthly_report.pdf', {
     *   ensureDirectory: true,
     *   mode: 0o640
     * });
     * if (atomicResult.isErr()) console.error('Failed to save:', atomicResult.error);
     */
    saveAtomic(filePath: string, options: PDFSaveOptions = {}) {
        return attemptAsync(async () => {
            PDF.assertAbsolutePath(filePath).unwrap();
            const parentDirectory = PDF.parentDirectory(filePath);

            if (options.ensureDirectory ?? true) {
                await fs.mkdir(parentDirectory, { recursive: true });
            }

            const temporaryPath = `${filePath}.tmp-${Date.now()}-${Math.random().toString(16).slice(2)}`;
            await fs.writeFile(temporaryPath, this.data, {
                mode: options.mode
            });
