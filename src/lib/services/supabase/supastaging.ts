...
type ConflictHandlerFn<Name extends Names, K extends keyof Row<Name>> = (conflict: Conflict<Name, K>) => Promise<Row<Name>[K]> | Row<Name>[K];

type SaveStrategy<Name extends Names, K extends keyof Row<Name>> = {
    strategy: 'ifClean' | 'force' | 'preferLocal' | 'preferRemote' | 'mergeClean' | 'manual' | ConflictHandlerFn<Name, K>;
}
...
export class SupaStaging<Name extends Names> extends WritableBase<Row<Name>> {
        ...
        return new Proxy({...data}, {
            set: (target, prop, value) => {
                if (this.config.static?.includes(prop as keyof Row<Name>)) {
                    console.warn(`Attempted to modify static field ${String(prop)}. This field is configured as static and cannot be modified.`);
                    return true;
                }
            ...
            deleteProperty: (_, property) => {
                throw new Error(`Attempted to delete property ${String(property)}. Deletion is not allowed in staging.`);
            }
        })
    }
    ...
    async pull(strategy: SaveStrategy<Name, keyof Row<Name>>) {
        ...
        for (const key in base) {
            ...
            } else if (!deepEqual(localValue, baseValue) && !deepEqual(remoteValue, baseValue) && !deepEqual(localValue, remoteValue)) {
        ...
        if (mergeState.status === 'conflict') {
            if (strategy.strategy === 'manual') {
                throw new Error(`Merge conflicts detected. Manual resolution required. Conflicts: ${JSON.stringify(mergeState.conflicts)}`);
