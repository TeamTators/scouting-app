...
> {
    ...
    initRealtime(config?: {
        ...
        channel
            .on('postgres_changes', { event: '*', schema: this.config.schema, table: this.table, filter: config?.filter }, (payload) => {
                ...
                switch (payload.eventType) {
                    ...
                    case 'UPDATE': {
                        ...
                        if (existing) {
                            ...
                            Object.assign(existing.data as any, validated);
    ...
    get(queryData: Partial<Row<Schema, RowName>>) {
        ...
        const satisfies = (data: SupaStructData<Schema, RowName>) =>
            Object.entries(queryData).every(([key, value]) => data.data[key as keyof Row<Schema, RowName>] === value);
        ...
        const paginateQuery = async (page: number, size: number) => {
            ...
            let query = this.supabase.schema(this.config.schema).from(this.table).select('*').range(from, to);
    ...
    getOR(queryData: Partial<Row<Schema, RowName>>) {
        ...
        const satisfies = (data: SupaStructData<Schema, RowName>) =>
            Object.entries(queryData).some(([key, value]) => data.data[key as keyof Row<Schema, RowName>] === value);

        const allQuery = async () => {
            let query = this.supabase.schema(this.config.schema).from(this.table).select('*');

            const orConditions = Object.entries(queryData)
    ...
    search(query: SearchQuery<Schema, RowName>) {
        ...
        const satisfies = (data: SupaStructData<Schema, RowName>): boolean => {
            const evaluate = (q: SearchQuery<Schema, RowName>): boolean => {
                if ('field' in q) {
                    const fieldValue = data.data[q.field];
                    switch (q.operator) {
                        case 'eq':
                        ...
                        case 'lte':
                            return fieldValue <= q.value;
                        case 'like':
                            return typeof fieldValue === 'string' && typeof q.value === 'string' && fieldValue.includes(q.value);
                        case 'ilike':
                            return typeof fieldValue === 'string' && typeof q.value === 'string' && fieldValue.toLowerCase().includes(q.value.toLowerCase());
                        default:
                            return false;
                    }
                } else if ('type' in q) {
        ...
        const paginateQuery = async (page: number, size: number) => {
            ...
            const queryBuilder = buildQuery(main, query).range(from, to);
    ...
    fromId(id: string) {
        return attemptAsync(async () => {
            const res = await this.supabase.schema(this.config.schema).from(this.table).select('*').filter('id', 'eq', id).single();
            const result = this.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'single').unwrap();
    ...
    new(data: Insert<Schema, Extract<RowName, InsertTableNames<Schema>>>) {
        return attemptAsync(async () => {
            const res = await this.supabase.schema(this.config.schema).from(this.table).insert(data as any).select('*').single();
            const result = this.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'single').unwrap();
    ...
    upsert(data: Insert<Schema, Extract<RowName, InsertTableNames<Schema>>>) {
        return attemptAsync(async () => {
            const res = await this.supabase.schema(this.config.schema).from(this.table).upsert(data as any).select('*').single();
            const result = this.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'single').unwrap();
    ...
    all() {
        ...
        const paginateQuery = async (page: number, size: number) => {
            const from = (page - 1) * size;
            const to = from + size - 1;
            const res = await this.supabase.schema(this.config.schema).from(this.table).select('*').range(from, to);
            const result = this.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'array').unwrap();
...
class SupaQuery<Schema extends RowSchemaName, RowName extends RowTableNames<Schema>> {
    ...
    constructor(
        ...
        private readonly paginateQuery: (page: number, size: number) => Promise<PaginatedResponse<SupaStructData<Schema, RowName>>>
    ) {}
     ...
     */
    then(
        onfulfilled?: ((value: Result<SupaStructData<Schema, RowName>[], Error>) => void | PromiseLike<void>) | null,
    ) {
        this._loading = true;
        return this.fetchAll()
            ...
            .catch((err) => {
                this._loading = false;
                const result = new Err(err instanceof Error ? err : new Error(String(err))) as Result<SupaStructData<Schema, RowName>[], Error>;
                onfulfilled?.(result);
                return result;
            });
    }
...
class SupaPagination<Schema extends RowSchemaName, RowName extends RowTableNames<Schema>> {
     ...
     */
    get hasNext() { return this._currentPage < this.pages; }
    /**
     ...
     */
    get hasPrev() { return this._currentPage > 1; }
    ...
    private executeFetch() {
        return this.paginateQuery(this._currentPage, this._pageSize)
            .then((res) => {
                ...
                this._currentPageIds = res.data.map(item => String(item.data.id));
                
                return new Ok(res.data);
            })
            .catch((err) => {
                return new Err(err instanceof Error ? err : new Error(String(err))) as Result<SupaStructData<Schema, RowName>[], Error>;
            });
    }
...
> {
    public readonly data: Row<Schema, RowName> = $state({} as any);
    ...
    update(updates: Partial<Insert<Schema, UpdateName>>) {
        return attemptAsync(async () => {
            ...
            const result = this.struct.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'single').unwrap();
            Object.assign(this.data as any, result);
            return this;
        });
    }
