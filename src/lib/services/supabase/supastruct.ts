...
if (browser) Object.assign(window, { supabase });

export type Table<Name extends keyof Database[SchemaName]['Tables']> = Database[SchemaName]['Tables'][Name];
...
export class SupaStruct<Name extends Names> {
    ...
    constructor(public readonly name: Name, public readonly config: SupaConfig = {}) {
        this.channel = supabase.channel(`struct-${schemaName}.${name}`);
        this.setupListeners();
    }
    ...
    private readonly registeredArrays = new Map<string, {
        array: SupaStructArray<Name>;
        satisfies: (data: PartialRow<Name>) => boolean;
    }>();

    registerArray(name: string, array: SupaStructArray<Name>, satisfies: (data: PartialRow<Name>) => boolean) {
        if (browser) this.registeredArrays.set(name, {
            array,
            satisfies,
        });
    }
    ...
    Generator(row: unknown) {
        ...
        this.cache.set(String(validated.id), data);
        return data;
    }
    ...
    validate(data: unknown) {
        ...
        if (!parseResult.success) {
            throw new Error(`Failed to validate data for table ${this.name}: ` + parseResult.error.message);
        }
    ...
    all() {
        ...
        const get = async () => {
            const res = this.runTransaction(await supabase.schema(schemaName).from(this.name).select('*'), 'array');
            if (res.isErr()) {
    ...
    new(...data: Insert<Name>[]) {
        ...
        supabase.schema(schemaName).from(this.name).insert(data as any).select('*').then(res => {
    ...
    fromId(id: Row<Name>['id']) {
        return attemptAsync(async () => {
            const res = await supabase.schema(schemaName).from(this.name).select('*').eq('id', id as any).single();
            const transactionResult = this.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'single');
    ...
    get(data: Partial<Row<Name>>) {
        ...
        const get = async () => {
            let query = supabase.schema(schemaName).from(this.name).select('*');
            ...
            if (transactionResult.isErr()) {
                this.log(`Error fetching with get query ${JSON.stringify(data)} from ${this.name}:`, transactionResult.error);
                return [];
            }
    ...
    search(query:  SearchQuery<Name>) {
        ...
        const main = supabase.schema(schemaName).from(this.name).select('*');
        ...
        const buildQuery = (base: typeof main, q: SearchQuery<Name>): typeof main => {
            ...
            if (q.type === 'or') {
                const orConditions = q.conditions.map(condition => {
        ...
        const get = async () => {
            ...
            if (transactionResult.isErr()) {
                this.log(`Error fetching with search query ${JSON.stringify(query)} from ${this.name}:`, transactionResult.error);
                return [];
            }
            return transactionResult.value;
        };
        ...
        this.registerArray(cacheKey, arr, (row) => {
            const evaluateQuery = (q: SearchQuery<Name>): boolean => {
                if ('field' in q) {
                    ...
                    switch (q.operator) {
                        case 'eq': return rowValue === q.value;
                        case 'neq': return rowValue !== q.value;
                        case 'gt': return rowValue > q.value;
                        case 'gte': return rowValue >= q.value;
                        case 'lt': return rowValue < q.value;
                        case 'lte': return rowValue <= q.value;
                        case 'like': return typeof rowValue === 'string' && new RegExp(String(q.value).replace(/%/g, '.*')).test(rowValue);
                        case 'ilike': return typeof rowValue === 'string' && new RegExp(String(q.value).replace(/%/g, '.*'), 'i').test(rowValue);
    ...
    archived() {
        ...
        const arr = this.arr();
        const get = async () => {
            const res = await supabase.schema(schemaName).from(this.name).select('*').eq('archived', true as any);
            const transactionResult = this.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'array');
    ...
    fromIds(ids: Row<Name>['id'][]) {
        ...
        const arr = this.arr();
        const get = async () => {
            const res = await supabase.schema(schemaName).from(this.name).select('*').in('id', ids as any);
            const transactionResult = this.runTransaction({
                data: res.data as any,
                error: res.error,
            }, 'array');
...
type SearchQuery<Name extends Names> = {
...
export class SupaStructData<Name extends Names> extends WritableBase<PartialRow<Name>> {
    constructor(public readonly struct: SupaStruct<Name>, data: PartialRow<Name>) {
        super(data);
    }
    ...
    update(fn: (data: PartialRow<Name>) => Update<Name>) {
...
export class SupaPagination<Name extends Names> extends WritableBase<{
    ...
    constructor(
        ...
        private readonly getter: (page: number, pageSize: number) => Promise<{
            rows: Row<Name>[];
            total: number;
            page: number;
            pageSize: number;
    ...
    private fetch(page: number, pageSize: number) {
        this.getter(page, pageSize).then(result => {
            this.set({
            ...
            console.error(`Failed to fetch page ${page} with page size ${pageSize} from table ${this.struct.name}:`, error);
        });
    }
