...
export const fullscreen = (element: HTMLElement) => {
	...
	attempt(() => {
		if (document['exitFullscreen']) {
			...
			Object.getOwnPropertyDescriptor(document, 'webkitExitFullscreen')?.value?.call(element);
		} else if (Object.prototype.hasOwnProperty.call(document, 'mozCancelFullScreen')) {
			Object.getOwnPropertyDescriptor(document, 'mozCancelFullScreen')?.value?.call(element);
		} else if (Object.prototype.hasOwnProperty.call(document, 'msExitFullscreen')) {
			Object.getOwnPropertyDescriptor(document, 'msExitFullscreen')?.value?.call(element);
		}
	});
