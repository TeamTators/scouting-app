<script lang="ts">
    ...
    onMount(() => {
        AppData.getEvent(eventKey).then(res => {
            ...
            matchData = res.value.matches.find(m => m.comp_level === compLevel && m.match_number === match) ?? null;
            if (matchData) {
...
<div class="container">
    ...
    <div class="row">
        <div class="grid">
            {#each teams as team}
                <a href="/app/event/{eventKey}/team/{team.team_number}/match/{matchData?.comp_level || 'pr'}/{matchData?.match_number || 0}" class="grid-item btn btn-secondary">
                    {team.team_number}
                </a>
            {/each}
        </div>
