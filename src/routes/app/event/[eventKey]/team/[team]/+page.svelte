<script lang="ts">
	...
	import { AppData } from '$lib/model/app/new/data-pull';
    ...
    onMount(() => {
        AppData.getEvent(eventKey).then(res => {
            ...
            matches = res.value.matches.filter(m => m.alliances.red.team_keys.includes(`frc${team}`) || m.alliances.blue.team_keys.includes(`frc${team}`));
        });
    });
...
<div class="container">
    <div class="row">
        <div class="input-group mb-3">
            <input type="text" class="form-control" bind:value={match} placeholder="Enter match like: qm46" oninput={(event) => {
    ...
    <div class="row">
        <div class="grid">
            {#each matches as match}
                <a href="/app/event/{eventKey}/team/{team}/match/{match.comp_level}/{match.comp_level === 'sf' ? match.set_number : match.match_number}" class="grid-item btn btn-secondary">
                    {match.comp_level} {match.comp_level === 'sf' ? match.set_number : match.match_number}
                </a>
            {/each}
        </div>
    </div>
