<script lang="ts">
	...
	const arr = runTests();
	const success = arr.derived((arr) =>
		arr.filter((test) => test.data.success === true || !test.data.required)
	);
	const failed = arr.derived((arr) =>
		arr.filter((test) => test.data.success === false && test.data.required)
	);
